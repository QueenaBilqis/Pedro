...
const Register: React.FC = () => {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 relative overflow-hidden my-12">
      {/* Background Decor */}
      <div className="absolute top-1/4 -right-20 w-96 h-96 bg-primary/20 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-1/4 -left-20 w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="glass-card w-full max-w-md p-8 md:p-12 rounded-3xl shadow-2xl relative z-10 transition-all duration-500">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-widest">
            {step === 'details' ? 'Join the Future' : 'Verify Email'}
          </p>
        </div>
        ...
        {step === 'details' ? (
            <form onSubmit={handleDetailsSubmit} className="space-y-6 animate-fade-in">
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        <span className="material-icons absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm">lock_reset</span>
                        <input 
                            type="password" 
                ...
                <button 
                    type="submit"
                    disabled={isLoading}
                    className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold uppercase tracking-widest py-4 rounded-xl shadow-lg transition-all transform active:scale-95 flex items-center justify-center gap-2 hover:opacity-90"
                >
                    {isLoading ? (
                        <span className="w-5 h-5 border-2 border-white/30 border-t-white dark:border-slate-400/30 dark:border-t-slate-900 rounded-full animate-spin"></span>
                    ) : (
            ...
            <form onSubmit={handleVerificationSubmit} className="space-y-8 animate-fade-in">
                <div className="text-center space-y-2">
                    <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4">
                        <span className="material-icons text-primary text-2xl">mark_email_read</span>
                    </div>
                    <p className="text-sm text-slate-600 dark:text-slate-300">
                        We sent a code to <span className="font-bold text-slate-900 dark:text-white">{email}</span>.<br/>
